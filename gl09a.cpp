...
int main(){
	...
	ShaderProgram shaderProgram{
		...
		Shader{"TextureShaderMix.frag", GL_FRAGMENT_SHADER}
	};
	glUseProgram(shaderProgram);

    GLTexture bob = init_texture("bob.jpg");
	GLTexture odd = init_texture("odd.jpg");

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, bob);
	Uniform{"texture0"} = 0; // O uniform deve guardar o valor da unidade de textura
		
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, odd);
	Uniform{"texture1"} = 1;
